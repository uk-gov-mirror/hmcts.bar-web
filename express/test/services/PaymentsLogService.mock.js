...
class PaymentsLogServiceMock {
  ...
  searchPaymentsLogByDailySequenceId(dailySequenceId) {
    nock(`${barUrl}`)
      ...
      .query({ status: 'P', allPayInstructionId: dailySequenceId, dailySequenceId, chequeNumber: dailySequenceId, postalOrderNumber: dailySequenceId, authorizationCode: dailySequenceId })
      .reply(httpStatusCodes.OK, [
        {
  ...
  searchPaymentsLogByPostalOrderNumber(postalOrderNumber) {
    nock(`${barUrl}`)
      .get('/payment-instructions')
      .query({ status: 'P', allPayInstructionId: postalOrderNumber, dailySequenceId: '345678', chequeNumber: '345678', postalOrderNumber, authorizationCode: postalOrderNumber })
      .reply(httpStatusCodes.OK, [
        {
          payer_name: 'PostalOrderPayer',
          amount: 300000,
  ...
  searchPaymentsLogByChequeNumber(chequeNumber) {
    nock(`${barUrl}`)
      .get('/payment-instructions')
      .query({ status: 'P', allPayInstructionId: chequeNumber, dailySequenceId: '123456', chequeNumber, postalOrderNumber: '123456', authorizationCode: chequeNumber })
      .reply(httpStatusCodes.OK, [
        {
