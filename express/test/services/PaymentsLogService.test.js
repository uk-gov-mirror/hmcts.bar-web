...
describe('Test: PaymentsLogService', () => {
  ...
  it('searchPaymentsLog when query is a word', () => {
    const makeHttpRequest = function(...args) {
      expect(args[0].uri).to.equal('http://localhost:8080/payment-instructions?status=D,P,PA,V,A,TTB,C,REJ,RDM&payerName=John');
    };
  ...
  it('searchPaymentsLog when query is multiple word', () => {
    const makeHttpRequest = function(...args) {
      expect(args[0].uri).to.equal('http://localhost:8080/payment-instructions?status=D,P,PA,V,A,TTB,C,REJ,RDM&payerName=John Doe');
    };
    const paymentsLogService = new PaymentsLogService(makeHttpRequest);
  ...
  it('searchPaymentsLog when query is number', () => {
    const makeHttpRequest = function(...args) {
      expect(args[0].uri).to.equal('http://localhost:8080/payment-instructions?status=D,P,PA,V,A,TTB,C,REJ,RDM&allPayInstructionId=123456&chequeNumber=123456&dailySequenceId=123456&postalOrderNumber=123456&authorizationCode=123456');
    };
    const paymentsLogService = new PaymentsLogService(makeHttpRequest);
